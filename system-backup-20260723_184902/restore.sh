        ...
        case $platform in
            ...
            "mac")
                for pkg in "${translated_packages[@]}"; do
                    ...
                    brew install "$pkg" || sys_failed+=("$pkg")
                done ;;
            "wsl")
                for pkg in "${translated_packages[@]}"; do
        ...
        for file in config/packages.{cask,mas}; do
            ...
            while IFS= read -r pkg; do
                ...
                case $type in
                    ...
                    "mas")  command -v mas >/dev/null 2>&1 && { mas list 2>/dev/null | grep -q "^$pkg" || mas install "$pkg" || log_warning "Failed: $pkg"; } ;;
                esac
            done < "$file"
        done
    fi
...
main() {
    ...
    if [[ -f config/zsh_history && -s config/zsh_history ]]; then
        ...
        if [[ -f "$HOME/.zsh_history" && -s "$HOME/.zsh_history" ]]; then
            ...
            grep -vE "$HISTORY_FILTER" "$HOME/.zsh_history" > "$filtered"
