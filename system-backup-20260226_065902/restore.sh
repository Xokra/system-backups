    ...
    case $platform in
        ...
        "arch")
            ...
            if ! command -v yay >/dev/null 2>&1 && ! command -v paru >/dev/null 2>&1; then
                ...
                if git clone https://aur.archlinux.org/yay.git && cd "$temp_dir/yay"; then
...
install_nerd_fonts() {
    ...
    local version=$(curl -s https://api.github.com/repos/ryanoasis/nerd-fonts/releases/latest 2>/dev/null | grep -m1 '"tag_name"' | sed 's/.*"tag_name": *"\([^"]*\)".*/\1/' 2>/dev/null || true); version="${version:-v3.2.1}"
    ...
    if curl -fsSL -o "$temp_dir/Meslo.zip" "https://github.com/ryanoasis/nerd-fonts/releases/download/$version/Meslo.zip" 2>/dev/null; then
        case $platform in
            "wsl")
                if unzip -q Meslo.zip "*.ttf" 2>/dev/null && ls *.ttf >/dev/null 2>&1; then
                    ...
                    if [[ "$font_installed" != true ]]; then
                        if mkdir -p ~/.local/share/fonts 2>/dev/null && cp *.ttf ~/.local/share/fonts/ 2>/dev/null; then
            ...
            "mac")
                ...
                if unzip -q Meslo.zip "*.ttf" 2>/dev/null && ls *.ttf >/dev/null 2>&1; then
                    if mkdir -p ~/Library/Fonts && cp *.ttf ~/Library/Fonts/ 2>/dev/null; then
            ...
            "arch")
                ...
                if mkdir -p ~/.local/share/fonts 2>/dev/null && unzip -q Meslo.zip -d ~/.local/share/fonts/ 2>/dev/null; then
                    fc-cache -fv >/dev/null 2>&1 || true
...
configure_zsh() {
    # Note: platform arg not needed here but kept for consistent calling convention
...
install_all_packages() {
    ...
    if [[ ${#translated_packages[@]} -gt 0 ]]; then
        ...
        case $platform in
            ...
            "arch")
                ...
                for pkg in "${translated_packages[@]}"; do
                    ...
                    elif ! sudo pacman -S --noconfirm "$pkg" >/dev/null; then
            ...
            "wsl")
                for pkg in "${translated_packages[@]}"; do
                    ...
                    if dpkg -l "$pkg" 2>/dev/null | grep -q "^ii"; then
                        ...
                        sudo apt install -y "$pkg" >/dev/null || sys_failed+=("$pkg")
                    fi
        ...
        [[ ${#sys_failed[@]} -gt 0 ]] && log_warning "Failed system packages: ${sys_failed[*]+"${sys_failed[*]}"}"
    fi
    ...
    if [[ "$needs_lazygit_manual" == true ]] && ! command -v lazygit >/dev/null 2>&1; then
        ...
        local lg_version; lg_version=$(curl -s "https://api.github.com/repos/jesseduffield/lazygit/releases/latest" | grep -o '"tag_name":"[^"]*"' | cut -d'"' -f4 | sed 's/^v//' || true)
        if [[ -z "$lg_version" ]]; then

            log_warning "⚠️ Could not determine lazygit version, skipping install"

        else

            curl -Lo /tmp/lazygit.tar.gz "https://github.com/jesseduffield/lazygit/releases/download/v${lg_version}/lazygit_${lg_version}_Linux_x86_64.tar.gz" || { log_warning "⚠️ lazygit download failed"; return 0; }
            tar xf /tmp/lazygit.tar.gz -C /tmp lazygit || { log_warning "⚠️ lazygit extract failed"; return 0; }
            sudo install /tmp/lazygit /usr/local/bin || { log_warning "⚠️ lazygit install failed"; return 0; }
            log_success "✅ lazygit installed"
        fi
    elif [[ "$needs_lazygit_manual" == true ]]; then
        # Check if update needed

        local lg_latest; lg_latest=$(curl -s "https://api.github.com/repos/jesseduffield/lazygit/releases/latest" | grep -o '"tag_name":"[^"]*"' | cut -d'"' -f4 | sed 's/^v//' || true)
        local lg_current; lg_current=$(lazygit --version 2>/dev/null | grep -o 'version=[^,]*' | cut -d= -f2 || true)
        if [[ -n "$lg_latest" && "$lg_latest" != "$lg_current" ]]; then

            log_info "📦 Updating lazygit $lg_current → $lg_latest..."

            curl -Lo /tmp/lazygit.tar.gz "https://github.com/jesseduffield/lazygit/releases/download/v${lg_latest}/lazygit_${lg_latest}_Linux_x86_64.tar.gz" || { log_warning "⚠️ lazygit download failed"; return 0; }
            tar xf /tmp/lazygit.tar.gz -C /tmp lazygit || { log_warning "⚠️ lazygit extract failed"; return 0; }
            sudo install /tmp/lazygit /usr/local/bin || { log_warning "⚠️ lazygit install failed"; return 0; }
    ...
    if [[ "$needs_rustup_manual" == true ]] && ! command -v rustup >/dev/null 2>&1; then
        ...
        curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y || { log_warning "⚠️ rustup install failed"; return 0; }
    ...
    if [[ ${#all_lang_packages[@]} -gt 0 ]]; then
        ...
        [[ ${#lang_failed[@]} -gt 0 ]] && log_warning "Failed language packages: ${lang_failed[*]+"${lang_failed[*]}"}"
    fi
...
main() {
    ...
    # Always run from the backup directory so relative config/ paths work
    local script_dir
    script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

    cd "$script_dir" || { log_error "❌ Cannot cd to backup directory"; exit 1; }
    ...
    case $platform in
        "wsl") 
            ...
            DEBIAN_FRONTEND=noninteractive sudo apt update >/dev/null 2>&1 && DEBIAN_FRONTEND=noninteractive sudo apt upgrade -y >/dev/null 2>&1 || { log_warning "⚠️ System update failed, continuing..."; }
            ;;
        ...
        "mac") 
            ...
            ;;
        "arch") 
            sudo pacman -Syu --noconfirm >/dev/null || { log_warning "⚠️ System update failed, continuing..."; }
            ;;
    esac
